    ...
    try {
        ...
        try {
            ...
            if (!adminStatus.isBotAdmin) {
                await sock.sendMessage(chatId, {
                    text: '🔒 *SPAIDO BOT* needs admin powers to swing the demotion hammer! 🔨'
                });
                return;
            }
            ...
            await sock.sendMessage(chatId, {
                text: '⚠️ SPAIDO BOT can’t check admin status. Make sure the bot has permissions.'
            });
            return;
        }
        ...
        if (error.data === 429) {
            ...
            try {
                await sock.sendMessage(chatId, {
                    text: '⚠️ Rate limit reached! Let SPAIDO BOT catch its breath. 😮‍💨'
                });
            } catch (retryError) {
            ...
            try {
                await sock.sendMessage(chatId, {
                    text: '❌ SPAIDO BOT failed to demote! Is the user already powerless or bot not admin?'
                });
            } catch (sendError) {
...
async function handleDemotionEvent(sock, groupId, participants, author) {
    try {
        ...
        const demotionMessage = `💢 *AUTO-DEMOTION DETECTED BY SPAIDO BOT* 💢\n\n` +
