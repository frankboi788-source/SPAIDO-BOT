...
async function promoteCommand(sock, chatId, mentionedJids, message) {
    ...
    if (userToPromote.length === 0) {
        await sock.sendMessage(chatId, {
            text: '🫵 Tag a user or reply to their message to *promote them with SPAIDO BOT power!* 🚀'
        });
        return;
    }
...
Use them wisely or prepare for *DEMODIFICATION* 🔨`;
        ...
        await sock.sendMessage(chatId, {
            text: '❌ SPAIDO BOT failed to promote the user(s). Maybe try again after praying. 😅'
        });
    }
}
...
async function handlePromotionEvent(sock, groupId, participants, author) {
    try {
        ...
        const promotionMessage = `
🔔 *SPAIDO BOT DETECTED A PROMOTION!* 🔔
